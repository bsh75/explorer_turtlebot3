...
# ---- Helper Functions ---- (These could be in a separate module for better organization)
...
class ExplorationNode:
    ...
    def find_goal(self):
        ...
        while search_size_cells <= max(costmap.info.width, costmap.info.height):
            ...
            if unexplored_areas:
                # Select closest goal
                goal_x, goal_y = min(unexplored_areas, key=lambda loc: self._distance_to_robot(loc))  
                rospy.loginfo(f"Selected goal: ({goal_x}, {goal_y})")
                self.add_coord_markers([(goal_x, goal_y)], size=3, shape=Marker.SPHERE)
        ...
        return None  # Indicate no goal found
